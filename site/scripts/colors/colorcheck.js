    ...
    if ((urlParams.get('fore') || urlParams.get('customfore')) && (urlParams.get('back') || urlParams.get('customback'))) {
        if (urlParams.get('fore')) {
            urlParams.get('fore').split(',').forEach(element => {
                document.getElementById('fore-#' + element.toLowerCase()).checked = true;
            });
        }
        if (urlParams.get('back')) {
            urlParams.get('back').split(',').forEach(element => {
                document.getElementById('back-#' + element.toLowerCase()).checked = true;
            });
        }
...
function checkColorsFull() {
    const backgroundList = Array.from(document.querySelectorAll('#background input:checked')).map(e => e.value);
    const foregroundList = Array.from(document.querySelectorAll('#foreground input:checked')).map(e => e.value);
    const customForeground = document.getElementById('customcolor-foreground').value;
    const customBackground = document.getElementById('customcolor-background').value;
