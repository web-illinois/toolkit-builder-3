...
## Overview
The JSON files are broken down into two categories:
* `builder\xxxxxxxxx.json`: this is a "page" in the builder to test a single CSS stylesheet, web component, or pattern. You can have multiple of these files in a repository.
* `builder\versions\xxxxxxxxx.json`: this is a template for the builder to create a UI. You no longer need to create one of these for each version, just if there are additional classes, attributes, or CSS variables you want to call out for the end user. 

## Component JSON file notes (what is in builder\xxxxxxxxx.json)
* `title`: The title of the builder item -- this is displayed in the Builder page for the component.
* `id`: This is a unique identifier used by the builder. If you have multiple builder files, these need to be unique. This is hidden to the end user except for the URL generated by the Builder. 
* `type`: "Web Component", "Stylesheet", or "Pattern". Others may be created in the future. 
...
* `path`: The path of the custom template in the builder application. This should match the permalink in your markdown file.
...
## Component Version JSON file notes (what is in builder\versions\xxxxxxxxx.json)
